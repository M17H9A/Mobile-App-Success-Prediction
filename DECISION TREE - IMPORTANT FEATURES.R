#---------------------------------------------Decision Tree(Oversampled and Undersampled-Important Features)----------------------------------------------------------------------
library(caret)
library(pROC)
library(rpart)

model_name <- 'DECISION TREE-KFold(oversampled-Important Features)'
set.seed(123)
...
# Fitting Decision Tree model
  decision_tree_model <- rpart(successful_app ~ rating_count_tot + rating_count_ver +
                                 user_rating_ver +
                                 ipadSc_urls.num + 
...
plot(roc_curve, main = "DT-OverSampled Data-ROC Curve(Important Features)", col = "blue", lwd = 2)
...
model_name <- 'DECISION TREE-KFold(undersampled-Important Features)'
set.seed(123)
...
# Ploting ROC curve for the last fold 
plot(roc_curve, main = "DT-UnderSampled Data-ROC Curve(Important Features)", col = "blue", lwd = 2)
